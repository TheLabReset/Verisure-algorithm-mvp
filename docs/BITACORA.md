...
**Gate 1 (subagente fresco): APROBADO CON CORRECCIONES** — 0 BLOQUEANTE, 0 MAYOR, 2 MENOR + 2 SUGERENCIA. Cross-check independiente ejecutado: 43/43 tests · chunks por módulo confirmados (Radar 16,5 / MAIA 12,6 / Demanda 8,5 / Contexto 7,3 KB + index 159,6) · Basic Auth validado en 3 ramas + timing-safe + sin credenciales en repo (9/9 asserts, lógica ejecutada con `Netlify.env`/`btoa` mockeados) · fuentes cargadas con 0 requests externos · las 4 vistas con cifras doradas y 0 errores · secciones por vista ≤7 · estados demo diseñados · `reduce` apaga la carita.

**Hallazgos Gate 1.** #M1 MENOR (exigible) `public/data/algorithm.json` commiteado desincronizado del pipeline: traía "1 **pts** menos" (pre-fix de plural F4); `npm run pipeline` produce "1 **pt** menos" → fallaba el check de reproducibilidad del gate. No visible en la app (el frontend re-deriva el brief con `composeBrief`), pero el contrato `/data` para el BI de Verisure mostraría el plural viejo. #M2 MENOR docs v1 residuales (`PRODUCTION_AUDIT`, `APIFY_SCRAPERS`, …) describen la arquitectura "Social Intelligence"/Apify retirada. #S1 SUGERENCIA placeholder del chat en `--ink-3`. #S2 SUGERENCIA el shimmer del `.skeleton` no respetaba `prefers-reduced-motion`.

**Resoluciones Gate 1.**
- **#M1 (MENOR).** `npm run pipeline` regenera el snapshot; `algorithm.json` ahora trae "1 pt menos". Reproducibilidad restaurada (segunda corrida sin diff adicional). El determinismo run-a-run ya estaba sano; solo faltaba re-publicar tras el fix de plural.
- **#M2 (MENOR).** Eliminados 6 docs v1 (`APIFY_SCRAPERS`, `API_REQUIREMENTS`, `API_SETUP_GUIDE`, `ML_ARCHITECTURE_PLAN`, `PRODUCTION_AUDIT`, `SCRAPERS_GUIDE`) — 0 menciones de Integrametrics, sin referencias desde README/código; la demolición de v1 ya queda registrada en `AUDITORIA-ALGORITHM.md`. Docs restantes = set autoritativo v2 (DESIGN, blueprint, BITACORA, AUDITORIA, PDF de la API).
- **#S2 (SUGERENCIA, adoptada).** `@media (prefers-reduced-motion: reduce) { .skeleton { animation: none } }` — el skeleton queda estático sin parpadeo.
- **#S1 (SUGERENCIA, no adoptada — justificada).** El placeholder se mantiene en `--ink-3`: es texto de *pista* (no de contenido), el input lleva `aria-label` y `:focus-visible`, y WCAG trata los placeholders con criterio distinto al texto de contenido. Subirlo a `--ink-2` (6,6:1) lo haría lucir como campo lleno (confusión vacío/relleno). Dos verificadores lo calificaron SUGERENCIA/nota, no incumplimiento AA. Se conserva por decisión de diseño.

**Auto-verificación Gate 1.** 43 tests verdes · build ok · snapshot regenerado (reproducible) · skeleton estático bajo `reduce` · docs v1 fuera · `package.json`/`lock` intactos.

**Criterio de gate.** Gate 1 sin bloqueante/mayor; los 2 menores resueltos, 1 sugerencia adoptada y 1 justificada por escrito (protocolo §1.4, precedente F1–F4). Se procede al merge — **última fase**.
